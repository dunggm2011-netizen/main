...
#  CẤU HÌNH — SỬA Ở ĐÂY
# ─────────────────────────────────────────────
BOT_TOKEN = "8619392859:AAGah_Fb0upGal4sw7lw4aZVHR_6qYDqVMc"
...
async def global_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── QUAN TRỌNG: Gọi lần lượt tất cả handlers ──
    # Mỗi handler sẽ trả về True nếu đã xử lý tin nhắn.
    # Nếu handler nào trả về False, tiếp tục gọi handler tiếp theo.
    # Điều này đảm bảo không có module nào bị bỏ sót.

    # 1. Admin handlers
    if await adm.handle_message(update, ctx): return

    # 2. User handlers (key, nạp tiền, ...)
